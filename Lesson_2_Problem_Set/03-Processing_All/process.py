...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in soup.find('table', class_="dataTDRight").find_all('tr', class_="dataTDRight"):
            try:
                data_row = [int(value.text.replace(",", "")) for value in row.find_all('td')]
            except ValueError as e:
                pass
            else:
                entry = dict(info)
                entry.update({"year": data_row[0], "month": data_row[1],
                              "flights": {"domestic": data_row[2], "international": data_row[3]}})
                # print entry
                data.append(entry)
