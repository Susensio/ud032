...
def main():
    ...
    query = "First Aid Kit"
    results = query_by_name(ARTIST_URL, query_type["simple"], query)
    filtered = [artist for artist in results["artists"] if artist["name"].upper() == query.upper()]
    pretty_print(filtered)
    pretty_print(len(filtered))
    print type(filtered[0]["name"])
